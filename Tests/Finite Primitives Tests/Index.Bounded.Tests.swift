...
struct `Index_Bounded - Type Structure` {
    ...
    func `typealias resolves to double-tagged`() {
        ...
        let idx: Index<Int>.Bounded<8> = Tagged(__unchecked: (), finite)
...
struct `Index_Bounded - Bridging` {
    ...
    func `narrowing init returns Some for valid indices`(value: Int) {
        let index: Index<Int> = Index(__unchecked: (), Ordinal(UInt(value)))
    ...
    @Test(arguments: [0, 1, 2, 3, 7])
    func `map rawValue produces same result as widening init`(value: Int) {
        let index: Index<Int> = Index(__unchecked: (), Ordinal(UInt(value)))
        let bounded: Index<Int>.Bounded<8> = .init(index)!
        let viaInit: Index<Int> = .init(bounded)
        let viaMap = bounded.map { $0.rawValue }
        #expect(viaInit == viaMap)
    }
}

// MARK: - Index.Bounded - Arithmetic via rawValue

@Suite
struct `Index_Bounded - Arithmetic` {
    @Test
    func `successor via rawValue`() {
        let finite: Ordinal.Finite<8> = 3
        let bounded: Index<Int>.Bounded<8> = Tagged(__unchecked: (), finite)
        let next = bounded.rawValue.successor()
        #expect(next != nil)
    ...
    @Test
    func `predecessor via rawValue`() {
        let finite: Ordinal.Finite<8> = 3
        let bounded: Index<Int>.Bounded<8> = Tagged(__unchecked: (), finite)
        let previous = bounded.rawValue.predecessor()
        #expect(previous != nil)
    ...
    @Test
    func `offset via rawValue`() {
        let finite: Ordinal.Finite<8> = 3
        let bounded: Index<Int>.Bounded<8> = Tagged(__unchecked: (), finite)
        let offset = bounded.rawValue.offset(by: 2)
...
struct `Index_Bounded - Edge Cases` {
    ...
    func `narrowing with zero capacity always returns nil`() {
        let index: Index<Int> = Index(__unchecked: (), .zero)
    ...
    func `narrowing with capacity 1 accepts only zero`() {
        let zero: Index<Int> = Index(__unchecked: (), .zero)
        let one: Index<Int> = Index(__unchecked: (), Ordinal(UInt(1)))
